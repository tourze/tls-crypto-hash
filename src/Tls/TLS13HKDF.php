...
namespace Tourze\TLSCryptoHash\Tls;
...
class TLS13HKDF
{
    ...
    /**
     * 当前使用的哈希算法（sha256 或 sha384）
     */
    private string $hashAlgorithm = self::DEFAULT_HASH;
    ...
    /**
     * 设置哈希算法（sha256|sha384）
     */
    public function setHashAlgorithm(string $hashAlgorithm): void
    {
        $algo = strtolower($hashAlgorithm);
        $this->hashAlgorithm = in_array($algo, ['sha256', 'sha384'], true) ? $algo : self::DEFAULT_HASH;
        $this->hashLength = 'sha384' === $this->hashAlgorithm ? 48 : 32;
    }
    ...
    public function extract(string $salt, string $ikm): string
    {
        ...
        return hash_hmac($this->hashAlgorithm, $ikm, $salt, true);
    }
    ...
    public function expand(string $prk, string $info, int $length): string
    {
        ...
        while (strlen($result) < $length) {
            $t = hash_hmac($this->hashAlgorithm, $t . $info . chr($counter), $prk, true);
            $result .= $t;
            ++$counter;
        }
    ...
    public function deriveSecret(string $secret, string $label, string $messages): string
    {
        ...
        if ('' === $messages) {
            // 如果消息为空，使用空字符串的哈希值
            $transcriptHash = hash($this->hashAlgorithm, '', true);
        } else {
            $transcriptHash = hash($this->hashAlgorithm, $messages, true);
        }
    ...
    public function deriveEarlySecret(string $psk = ''): string
    {
        ...
        if ('' === $psk) {
            $psk = str_repeat("\0", $this->hashLength);
        }

        $salt = str_repeat("\0", $this->hashLength); // 初始盐值为全0填充
    ...
    public function deriveMasterSecret(string $handshakeSecret): string
    {
        ...
        $zeroKey = str_repeat("\0", $this->hashLength);
